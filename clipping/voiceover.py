...
async def _synthesize_async(text: str, voice: str, output_audio_path: str, output_subs_path: str = None):
    ...
    if output_subs_path:
        ...
        # Parse the generated VTT into our internal segment format
        return _parse_vtt_to_segments(output_subs_path)
    return []

def _parse_vtt_to_segments(vtt_path: str) -> list[dict]:
    """Convert WebVTT generated by edge-tts into the internal segment format."""
    segments = []
    ...
    for line in lines:
        ...
        if "-->" in line:
            ...
            if len(parts) == 2:
                ...
                end_s = _vtt_time_to_seconds(end_str)
                current_seg = {"start": start_s, "end": end_s, "words": []}
        elif line and current_seg is not None and not line.startswith("WEBVTT"):
            # This is the text for the current timing
            word = line
            ...
            # edge-tts generates word-by-word VTT, so one word per block
            # Group into slightly larger sentences if needed, but word-level is fine for our renderer
            current_seg["text"] = word
