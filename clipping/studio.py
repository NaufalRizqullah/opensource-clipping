...
File ini adalah entry point utama untuk modul studio.
Logic rendering dipisahkan ke dalam sub-package `clipping.studio/`
untuk menjaga kebersihan kode.

Aturan Refactor:
1. Jangan ubah logic asli.
2. `studio.py` hanya sebagai orchestrator (proses_klip) dan re-exporter.
"""
...
from .studio.subtitle import buat_file_ass

from .studio.hybrid import buat_video_hybrid

from .studio.split import buat_video_split_screen

from .studio.camera import buat_video_camera_switch

from .studio.extras import siapkan_glitch_video, buat_thumbnail
...
def proses_klip(
    ...
    start_c = metadata_klip["start"]
    end_c = metadata_klip["end"]
    label = f"Klip #{rank}"
    ...
    broll_data = metadata_klip.get("broll_data", [])
    ...
    buat_file_ass(
        ...
        typography_plan=metadata_klip.get("typography_plan", []),
